...
def blur(image, kernel_size):
    ...
    # this is a variable that tells us how many neighbors we actually
    # look at (ie for a kernel of 3, this value should be 1)
    for x in range(x_pixels):
        for y in range(y_pixels):
            for c in range(num_channels):
                # we are going to use a naive implementation of iterating through each neighbor and summing
                # there are faster implementations where you can use memoization, but
                # this is the most straightforward for a beginner to understand
                total = 0
...
def apply_kernel(image, kernel):
    ...
    # we actually look at (ie for a 3x3 kernel, this value should be 1)
    for x in range(x_pixels):
        for y in range(y_pixels):
            for c in range(num_channels):
                total = 0
