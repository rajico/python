...
class Sudoku():
    ...
    def __init__(self):
        ...
        try:
            ...
            print
            "Error: Imposible cargar tablero de juego"
            sys.exit(1)
        f.readline()
        fila = 0
        while fila < 9:
    ...
    while (not sudoku.finalizado() and opcion != "h"):
        ...
        if (opcion == "a"):
            ...
            if sudoku.fijar_valor(fila, columna, valor):
                ...
                mensaje = "Imposible fijar valor. Revise la jugada."
        elif opcion == "b":
            ...
            if sudoku.borrar_valor(fila, columna):
                ...
                mensaje = "Imposible borrar valor. Revise la jugada."
        elif opcion == "c":
            fila, columna, valor = Entrada()
            if sudoku.nueva_opcion(fila, columna, valor):
                ...
                mensaje = "Imposible agregar opción. Revise la jugada."
        elif opcion == "d":
            fila, columna, valor = Entrada()
            if sudoku.borrar_opcion(fila, columna, valor):
                ...
                mensaje = "Imposible borrar opción. Revise la jugada."
        elif opcion == "e":
            fila, columna, valor = Entrada()
            if sudoku.es_factible(fila, columna, valor):
                ...
                mensaje = "NO es factible introducir el valor " \
                          + str(valor) + " en la celda (" + str(fila) + "," \
                          + str(columna) + ")."
        elif opcion == "f":
            if sudoku.undo():
                mensaje = "Jugada deshecha."
            else:
                mensaje = "Imposible deshacer la jugada."
        elif opcion == "g":
            fila, columna = Entrada(False)
            ...
            mensaje = "Introduzca una opción correcta."
        os.system("clear")
    if opcion == "h": print
    "¡Adiós!"
    else:
        print("Sudoku completado. Enhorabuena.")
    return 0
